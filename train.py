...
def train_model(model, train_loader, valid_loader, optimizer, scheduler, num_epochs=25):
            ...
            if phase == 'train':
                for iter, (images, targets) in enumerate(model_loader):
                    ...
                    if (iter+1) % 100 ==0:
                        ...
                        print('iter:{} Loss: {:.4f} Time: {:.0f}m {:.0f}s'.format(iter, loss, 
                                                                                  iter_end//60, iter_end % 60))
                        iter_start = time.time()
                scheduler.step()
    ...
    print('Best val Acc: {:4f} Best val mAP: {:4f}'.format(best_loss, best_mAP))
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train_dataset, batch_size=cfg.batch_size, shuffle=True, num_workers=cfg.workers, collate_fn=collate_fn)
    valid_loader = DataLoader(valid_dataset, batch_size=1, shuffle=False, num_workers=cfg.workers, collate_fn=collate_fn)
    ...
    lr_scheduler = lr_scheduler.MultiStepLR(optimizer_ft, milestones=cfg.milestones, gamma=cfg.gamma)

    model_ft = train_model(model_ft, train_loader, valid_loader, optimizer_ft, lr_scheduler, num_epochs=cfg.epochs)
